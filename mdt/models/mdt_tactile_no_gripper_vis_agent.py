...
def print_model_parameters(model):
    ...
    for name, submodule in model.named_modules():
        ...
        if (
            "." not in name or name.count(".") <= 10
        ):  # Can be adjusted based on your model structure
...
class MDTAgent(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optim_groups.extend(
            [
                ...
                {
                    "params": self.gen_img.parameters(),
                    "weight_decay": self.optimizer_config.transformer_weight_decay,
                },
                {
                ...
                {
                    "params": self.left_tactile_resnet.parameters(),
                    "weight_decay": self.optimizer_config.transformer_weight_decay,
                },
                {
                    "params": self.right_tactile_resnet.parameters(),
                    "weight_decay": self.optimizer_config.transformer_weight_decay,
                },
            ]
        )
        optim_groups.extend(
            [
                {
                    "params": self.clip_proj.parameters(),
    ...
    def clip_extra_forward(self, perceptual_emb, latent_goal, actions, sigmas, noise):
        ...
        context = self.model.forward_context_only(
            perceptual_emb, noised_input, latent_goal, sigmas
        )
        return context

    def training_step(
        self, batch: Dict[str, Dict], batch_idx: int, dataloader_idx: int = 0
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            perceptual_emb, latent_goal, image_latent_goal = (
                self.compute_input_embeddings(dataset_batch)
            )
            ...
            if not isinstance(self.gen_img, NoEncoder):
                ...
                left_tactile_goal = dataset_batch["rgb_obs"]["gen_tactile"][:, :, :3, :, :]
                right_tactile_goal = dataset_batch["rgb_obs"]["gen_tactile"][:, :, 3:, :, :]
                img_gen_frame_diff = (
                    dataset_batch["future_frame_diff"]
                    if "future_frame_diff" in dataset_batch
                    else 3
                )
                # combine both goal images
                rgb_pred_goal = torch.cat(
                    [rgb_static_goal, left_tactile_goal, right_tactile_goal], dim=1
                )
                img_gen_embed = latent_encoder_emb
                img_gen_loss_part = self.compute_img_gen_loss(
                    img_gen_embed, rgb_pred_goal, img_gen_frame_diff=img_gen_frame_diff
                )
        ...
        self._log_training_metrics(
            action_loss, total_loss, cont_loss, img_gen_loss, total_bs
        )
        return total_loss
    ...
    def validation_step(
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            pred_loss = torch.nn.functional.mse_loss(
                action_pred, dataset_batch["actions"]
            )
            latent_encoder_emb = self.model.inner_model.latent_encoder_emb
    ...
    def compute_input_embeddings(self, dataset_batch):
        ...
        rgb_tactile = dataset_batch["rgb_obs"]["rgb_tactile"][:, :-1]
        ...
        lang_text = (
            dataset_batch["lang_text"] if "lang" in self.modality_scope else None
        )
        ...
        if "lang" in self.modality_scope:
            ...
            if self.use_text_not_embedding:
                latent_goal = self.language_goal(dataset_batch["lang_text"]).to(
                    rgb_static.dtype
                )
            else:
        ...
        perceptual_emb = self.embed_visual_obs(rgb_static, rgb_tactile)
        perceptual_emb["modality"] = self.modality_scope
        return perceptual_emb, latent_goal, image_latent_goal

    def embed_visual_obs(self, rgb_static, rgb_tactile):
        # reshape rgb_static and rgb_gripper
        rgb_static = einops.rearrange(rgb_static, "b t c h w -> (b t) c h w")
        rgb_tactile = einops.rearrange(rgb_tactile, "b t c h w -> (b t) c h w")
        rgb_tactile_left = rgb_tactile[:, :3, :, :]
        rgb_tactile_right = rgb_tactile[:, 3:, :, :]

        static_tokens = self.static_resnet(rgb_static)
        left_tactile_tokens = self.left_tactile_resnet(rgb_tactile_left)
        right_tactile_tokens = self.right_tactile_resnet(rgb_tactile_right)
        static_tokens = einops.rearrange(static_tokens, "b (t d) -> b t d", t=1)
        left_tactile_tokens = einops.rearrange(left_tactile_tokens, "b (t d) -> b t d", t=1)
        right_tactile_tokens = einops.rearrange(right_tactile_tokens, "b (t d) -> b t d", t=1)
        token_seq = {
            "static": static_tokens,
            "left_tactile": left_tactile_tokens,
            "right_tactile": right_tactile_tokens,
        }
        return token_seq
    ...
    def compute_img_gen_loss(
        ...
        img_gen_pred, mask, restore_idxs, visible_patches = self.gen_img(
            latent_embeddings, goal_img, img_gen_frame_diff
        )
        # 2. compute the loss
        img_gen_loss = self.gen_img.compute_loss(
            goal_img, img_gen_pred, mask, restore_idxs
        )
        if store_img:
            ...
            try:
                self.logger.experiment.log(
                    {
                        f"generated_img_{batch_idx}": wandb.Image(
    ...
    def compute_contrastive_loss(
        ...
        if "lang" in self.modality_scope:
            ...
            if is_distributed and self.use_distributed_clip:
                all_latent_vis_embed = self.all_gather(
                    latent_vis_embed, sync_grads=True
                )
                all_latent_language_embed = self.all_gather(
                    latent_language_embed, sync_grads=True
                )
                all_latent_language_embed = einops.rearrange(
                    all_latent_language_embed, "n b d -> (n b) d"
                )
                all_latent_vis_embed = einops.rearrange(
                    all_latent_vis_embed, "n b d -> (n b) d"
                )
            ...
            lang_text = (
                dataset_batch["lang_text"] if "lang_text" in dataset_batch else None
            )
            ...
            return torch.tensor(0.0).to(
                self.device
            )  # Return a zero tensor if "lang" is not in the modality scope

    def _log_training_metrics(
        self, action_loss, total_loss, cont_loss, img_gen_loss, total_bs
    ):
        """
    ...
    def _log_validation_metrics(self, pred_loss, img_gen_loss, val_total_act_loss_pp):
        ...
        self.log("val_act/img_gen_loss_pp", img_gen_loss, sync_dist=True)
    ...
    def denoise_actions(  # type: ignore
        ...
        x = (
            torch.randn((len(latent_goal), self.act_window_size, 7), device=self.device)
            * self.sigma_max
        )
    ...
    def forward(self, obs, goal):
        ...
        if "lang" in goal:
            ...
            if self.use_text_not_embedding:
                ...
                latent_goal = (
                    self.language_goal(goal["lang"])
                    .unsqueeze(0)
                    .to(torch.float32)
                    .to(obs["rgb_obs"]["rgb_static"].device)
                )
        else:
            modality = "vis"
            if self.use_delta_goal:
                perceptual_goal_emb = self.visual_goal(
                    obs["rgb_obs"]["rgb_static"].squeeze(0)
                )
            else:
                perceptual_goal_emb = self.visual_goal(
                    obs["rgb_obs"]["rgb_static"][:, -1]
                ).unsqueeze(1)  # [:, -1])
    ...
    def clip_auxiliary_loss(
        self, image_features, lang_features, mode="symmetric", lang_text=None
    ):
        ...
        if mode == "symmetric":
            similarity_matrix_lang_img = (
                logit_scale * lang_features @ image_features.t()
            )
            ...
            raise ValueError(
                "Invalid mode. Expected one of: 'symmetric', 'img_to_text', 'text_to_img'."
            )
        return infonce_loss
